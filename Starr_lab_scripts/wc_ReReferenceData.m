function [ecog, lfp, bad] = wc_ReReferenceData(ecog, lfp, ref_method, name)

bad=[];
if ref_method <2 % use common reference
    length_CAR = length(ecog.contact);
    data = nan*ones(length_CAR,length(ecog.contact(17).signal));
    % compute common ref and exclude bad channels
    for i = 1: length_CAR
        if ~isempty(ecog.contact(i).signal)
            data(i,:) = ecog.contact(i).signal;
        end
        if ~isempty(strfind(name,'theta')) || ~isempty(strfind(name,'stim')) || ~isempty(strfind(name,'contin')) || ~isempty(strfind(name,'DBS')) || ~isempty(strfind(name,'OFC'))
            bad = [bad];
        else
...
if ~isempty(lfp)
    ...
    lfp.contact(i).signal_ref=lfp.contact(i).signal-lfp.contact(1).signal;
end
